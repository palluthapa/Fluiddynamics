...
% equations for two fluid flow in an inclined plane
...
Mmax=110 ;                 % Number of nodes in Y-direction.
...
us_bar=0.15 ;              % Constant for Prandtl shift.
...
for j=3:Nmax
    ...
    while err>tol
       k=k+1;
        ...
        tn=unew(Mmax)-y(Mmax)-f(j)-(us_bar) ;
        tnp1=0 ;
        ...
        tnp1=unew(Mmax)-y(Mmax)-f(j)-(us_bar) ;
...
% velocity_u=reshape(u',(Nmax*Mmax),1);
% velocity_v=reshape(v',(Nmax*Mmax),1);
% Tangentialshear_omega=reshape(omega',(Nmax*Mmax),1);
% Stream_function=reshape(psi',(Nmax*Mmax),1);
% Pressure_Gradient=pprime';
% X_values=reshape(X',(Nmax*Mmax),1);
% Y_values=reshape(Y',(Nmax*Mmax),1);
% x_vector=x';
% y_vector=y';
