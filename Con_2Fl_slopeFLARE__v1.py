...
ro=1.5**3; mu=1.5**2; a0=.2; nu=mu/ro
Ka=10
...
#compute coefficients in momentum
    ...
    for k in range(mmax):
        if up[k] > 0:    
            bp[k]=-2.-1.5*dy*dy/dx*up[k]
            dp[k]=dy*dy/2./dx*up[k]*(-4.*wp[n-1,k]+wp[n-2,k])
        else:
            bp[k]=-2.
            dp[k]=0.
    for k in range(kmax):
        if um[k]>0:    
            bm[k]=-2.-1.5*HH*dz*dz/dx*um[k]
            dm[k]=HH/2.*dz*dz/dx*um[k]*(-4.*wm[n-1,k]+wm[n-2,k]) 
        else:
            bm[k]=-2.
            dm[k]=0.
